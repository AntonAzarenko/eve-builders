...
public class NavigationTab extends Tab {
    ...
    public NavigationTab(Class<? extends Component> navigationTarget, String label, AbstractIcon<?> icon, int badgeCount) {
        super();
        this.navigationTarget = navigationTarget;
        updateLabel(label, icon, badgeCount);
    }

    public void updateLabel(String label, AbstractIcon<?> icon) {
        updateLabel(label, icon, 0);
    }

    public void updateLabel(String label, AbstractIcon<?> icon, int badgeCount) {
        removeAll();
