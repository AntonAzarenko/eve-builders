package com.azarenka.evebuilders.domain.dto.acl;

import java.util.Collections;
import java.util.LinkedHashSet;
import java.util.Set;

public record UserAccessDto(String userId,
                            String username,
                            Set<RoleDto> roles,
                            Set<PermissionDto> directPermissions,
                            Set<PermissionDto> finalPermissions,
                            boolean superAdmin) {

    public UserAccessDto {
        roles = roles == null ? Set.of() : Collections.unmodifiableSet(new LinkedHashSet<>(roles));
        directPermissions = directPermissions == null ? Set.of() : Collections.unmodifiableSet(new LinkedHashSet<>(directPermissions));
        finalPermissions = finalPermissions == null ? Set.of() : Collections.unmodifiableSet(new LinkedHashSet<>(finalPermissions));
    }
}
